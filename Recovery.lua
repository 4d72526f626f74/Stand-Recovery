...
local function script_download()
    root:hyperlink("Download Script", "https://github.com/4d72526f626f74/Stand-Recovery")
end

if not filesystem.exists(lib_dir) then
    filesystem.mkdir(lib_dir)
    for _, i in pairs(required_files) do
        if not filesystem.exists(filesystem.scripts_dir() .. "lib/recovery" .. i) then
            script_download()
            while true do
                util.toast("Please download the script again, some files are missing")
                util.yield()
            end
            break
        end
